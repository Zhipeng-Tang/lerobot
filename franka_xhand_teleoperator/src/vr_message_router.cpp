...
public:
    VRMessageRouter(const VRRouterConfig& config = VRRouterConfig()) 
        ...
        // Wrist: "Right wrist:, x, y, z, qx, qy, qz, qw" with optional ", leftFist: state"
        wrist_pattern_ = std::regex(R"(Right wrist:,\s*([-\d\.]+),\s*([-\d\.]+),\s*([-\d\.]+),\s*([-\d\.]+),\s*([-\d\.]+),\s*([-\d\.]+),\s*([-\d\.]+)(?:\s*,\s*leftFist:\s*(\w+))?)");
    ...
    void parse_vr_messages(const std::string& message) {
        ...
        if (std::regex_search(message, wrist_match, wrist_pattern_)) {
            try {
                ...
                std::cout << "Error parsing wrist data: " << e.what() << std::endl;
            }
        }
