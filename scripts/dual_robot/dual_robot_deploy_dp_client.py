#!/usr/bin/env python3
"""
Diffusion Policy Client - Robot control with remote inference.
...
def parse_args():
    parser = argparse.ArgumentParser(description="Deploy DP Client")
...
def main():
    print("=== Combo Robot Policy Client ===")
    args = parse_args()
    ...
    cameras = {
        ...
        "wrist_view_view": RealSenseCameraConfig(
    ...
    try:
        while True:
            ...
            for i in range(7):
                ...
                #     env_state.append(action_chunk[chunk_idx-1][i])
                # else: 
                env_state.append(obs[f"arm_joint_{i}.pos"])
            ...
            # Keep only last 2 observations (n_obs_steps)
            n_obs_steps = 2
