...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Deploy DP")
    parser.add_argument("--ckpt-dirs", type=str, default=None, 
                        help="Checkpoint dirs (default: None)")
...
def main():
    ...
    # Load policy
    assert args.ckpt_dirs is not None, "Checkpoint dirs must not be none. "
    policy_path = Path(args.ckpt_dirs)  # USE YOUR POLICY PATH
    ...
    if stats_path.exists():
        ...
        stats = {}
        for key in ["observation.state", "observation.images.front_left_view", "observation.images.wrist_view", "observation.images.head_view", "action"]:
            if key in raw_stats:
                stats[key] = {}
