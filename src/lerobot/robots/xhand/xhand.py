...
class XHand(Robot):
    ...
    def _connect_ethercat(self) -> None:
        """Connect via EtherCAT network interface.
        
        EtherCAT connection requires:
        1. XHand connected via Ethernet cable to the specified interface
        2. Proper network configuration (IP in same subnet or direct connection)
        3. Root privileges for raw socket access
        
        Args:
            self.config.ethercat_interface: Network interface name (e.g., "eth0", "enp3s0")
        """
        import subprocess
        import os
        
        ethercat_interface = self.config.ethercat_interface
        
        logger.info(f"Opening EtherCAT connection on interface '{ethercat_interface}'")
        ...
        try:
            ...
            if result.returncode != 0:
                raise ConnectionError(f"Network interface '{ethercat_interface}' not found")
            logger.info(f"Found network interface: {ethercat_interface}")
        except FileNotFoundError:
            logger.warning("ip command not available, skipping interface verification")
        
        # Open EtherCAT device
        error_struct = self._device.open_ethercat(ethercat_interface)
        
        if error_struct.error_code != 0:
            error_msg = error_struct.error_message
            
            # Provide helpful error messages
            if "No socket connection" in error_msg or "Excecute as root" in error_msg:
                logger.error(f"EtherCAT connection requires root privileges.")
                logger.error(f"Please run with sudo: sudo python3 {__file__}")
                raise ConnectionError(
                    f"EtherCAT connection failed: {error_msg}. "
                ...
                try:
                    result = subprocess.run(
                        ['ip', 'addr', 'show', ethercat_interface],
                        capture_output=True, text=True
                    )
                    if result.returncode == 0 and 'inet ' in result.stdout:
                        logger.warning(f"Interface {ethercat_interface} has IP but no XHand detected.")
                        logger.warning("Please verify:")
                        logger.warning("  1. XHand is powered on")
                        logger.warning("  2. Ethernet cable is properly connected")
                        logger.warning("  3. XHand and PC are in the same network (e.g., 172.16.0.x)")
                    else:
                        logger.warning(f"Interface {ethercat_interface} may not be configured.")
                        logger.warning("Configure with: sudo ip addr add 172.16.0.1/24 dev {ethercat_interface}")
                except Exception:
                    pass
                
                raise ConnectionError(
                    f"No XHand device found on interface '{ethercat_interface}'. "
                    f"Check connection and network configuration."
                )
            else:
                raise ConnectionError(f"Failed to open EtherCAT: {error_msg}")
        
        logger.info("EtherCAT connection opened successfully")
        
        # Wait a moment for EtherCAT bus to settle
        time.sleep(0.5)
        
        # Try to enumerate devices first
        try:
            devices = self._device.enumerate_devices(ethercat_interface)
            logger.info(f"Enumerated devices on {ethercat_interface}: {devices}")
        except Exception as e:
            logger.debug(f"enumerate_devices failed: {e}")
        
        # Get hand ID - enumerate or use configured value
        hands_id = self._device.list_hands_id()
        logger.info(f"list_hands_id returned: {hands_id}")
        
        if hands_id:
            self._hand_id = hands_id[0]
            logger.info(f"Found XHand with ID: {self._hand_id}")
        else:
            # If no hands found but connection succeeded, try using configured ID
            if self._hand_id >= 0:
                logger.info(f"No hands auto-detected, using configured hand ID: {self._hand_id}")
                # Try to read state to verify connection
                try:
                    error_struct, state = self._device.read_state(self._hand_id, True)
                    if error_struct.error_code == 0:
                        logger.info(f"Successfully communicated with hand ID {self._hand_id}")
                    else:
                        logger.warning(f"Could not communicate with hand ID {self._hand_id}: {error_struct.error_message}")
                except Exception as e:
                    logger.warning(f"Failed to verify hand ID {self._hand_id}: {e}")
            else:
                raise ConnectionError(
                    f"No XHand devices found on interface '{ethercat_interface}'. "
                    f"Check that XHand is powered on and properly connected via Ethernet."
                )
